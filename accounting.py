...
# expected payment is equal to number of melon bounght times cost of one melon
# if expected payment does not equal to actual payment, print customer name, and how much
# he's actually paid (up to 2 decimals) and how much he's expected to pay
...
def customer_payment(the_file):
    for line in the_file:
        ...
        melon = words[2]
        paid = float(words[3])
        ex_paid = float(int(melon))
        if paid != ex_paid:
            print name, "paid ", paid, " but expected to pay ", ex_paid
        else:
            print name, "paid correctly!"
